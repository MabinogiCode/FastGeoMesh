...
namespace FastGeoMesh.Domain {
    ...
    public sealed class MeshAdjacency {
        ...
        /// <summary>
        /// Neighbor quad indices for each quad (array of length 4, -1 if no neighbor).
        /// Each element is an array of four integers representing neighbor quad indices for the four edges.
        /// </summary>
        private readonly int[][] _neighbors;

        /// <summary>Gets the neighbor quad indices for each quad (array of length 4, -1 if no neighbor).</summary>
        public IReadOnlyList<int[]> Neighbors => _neighbors;

        /// <summary>
        /// Backing collection of boundary edge tuples (a,b) where edge is incident to a single quad.
        /// </summary>
        private readonly List<(int a, int b)> _boundaryEdges = new();

        /// <summary>Gets the collection of boundary edges (edges incident to a single quad).</summary>
        public ReadOnlyCollection<(int a, int b)> BoundaryEdges => _boundaryEdges.AsReadOnly();

        /// <summary>
        /// Backing collection of non-manifold edges (edges incident to more than two quads).
        /// </summary>
        private readonly List<(int a, int b)> _nonManifoldEdges = new();

        /// <summary>Gets the collection of non-manifold edges (edges incident to more than two quads).</summary>
